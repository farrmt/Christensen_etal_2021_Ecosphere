...
data <- list()
data[[1]] <- read.csv("~/Christensen_etal/RawData/Aerial2014xy.csv", header = TRUE)
data[[2]] <- read.csv("~/Christensen_etal/RawData/Aerial2016xy.csv", header = TRUE)
data[[3]] <- read.csv("~/Christensen_etal/RawData/StartPoint.csv")
...
for(u in 1:7){
  ...
  inits <- function(){list(Nx = Nst, Ny = Nst, gamma = runif(1, 5, 6),
  ...
  S[[u]] <- jagsUI(jags.data, inits, params, model = "~/Christensen_etal/DataAnalysis/Revision.txt",
  ...
  tryCatch({Out[1,7,u] <- S[[u]]$sd$mean.Abundx[1]}, error = function(e){})
  tryCatch({Out[1,8,u] <- S[[u]]$sd$mean.Abundx[2]}, error = function(e){})
  tryCatch({Out[1,9,u] <- S[[u]]$sd$mean.Abundy[1]}, error = function(e){})
  tryCatch({Out[1,10,u] <- S[[u]]$sd$mean.Abundy[2]}, error = function(e){})
  tryCatch({Out[1,11,u] <- S[[u]]$sd$pi}, error = function(e){})
  tryCatch({Out[1,12,u] <- S[[u]]$sd$p}, error = function(e){})
  tryCatch({Out[1,13,u] <- all(unlist(S[[u]]$Rhat) < 1.1 | is.na(unlist(S[[u]]$Rhat)))}, error = function(e){})
